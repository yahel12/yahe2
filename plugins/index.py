...
async def index_files(bot, query):
    ...
    try:
        # Split the data and ensure it has the expected number of parts
        parts = query.data.split("#")
        if len(parts) != 5:
            raise ValueError(f"Unexpected number of parts in query.data: {len(parts)} (expected 5)")
...
async def set_skip_number(bot, message):
    ...
    if len(parts) == 2:
        ...
        try:
            ...
            await message.reply("Error: Skip number should be an integer.")
    else:
        await message.reply("Error: Please provide a skip number. Usage: /setskip <number>")
...
async def index_files_to_db(lst_msg_id, chat, msg, bot):
    ...
    async with lock:
        try:
            ...
            temp.CANCEL = False  # Reset the cancel flag
